...
class GFFormDisplay{
    ...
    public static function get_form($form_id, $display_title=true, $display_description=true, $force_display=false, $field_values=null, $ajax=false, $tabindex = 1){
        ...
        if(empty($confirmation_message)){
            ...
            $form_string .= "
             ...
             if($has_pages){
                $previous_button = self::get_form_button($form["id"], "gform_previous_button_{$form["id"]}", $form["lastPageButton"], __("Previous", "gravityforms"), "button gform_previous_button btn", __("Previous Page", "gravityforms"), self::get_current_page($form_id) -1);
    ...
    private static function get_form_button($form_id, $button_input_id, $button, $default_text, $class, $alt, $target_page_number){
        ...
        if($button["type"] == "text" || empty($button["imageUrl"])){
            ...
            $button_input= "<input type='image' src='{$imageUrl}' id='{$button_input_id}' class='gform_image_button btn' alt='{$alt}' {$tabindex} {$onclick}/>";
        }
        return $button_input;
    }
    ...
    private static function gform_footer($form, $class, $ajax, $field_values, $previous_button, $display_title, $display_description){
        ...
        $footer = "
        <div class='" . $class ."'>";
        $button_input = self::get_form_button($form["id"], "gform_submit_button_{$form["id"]}", $form["button"], __("Submit", "gravityforms"), "button gform_button btn", __("Submit", "gravityforms"), 0);
    ...
    public static function get_field($field, $value="", $force_frontend_label = false, $form=null, $field_values=null){
        ...
        if($field["type"] == "page"){
            if(IS_ADMIN && RG_CURRENT_VIEW == "entry"){
                ...
                $next_button = self::get_form_button($form["id"], "gform_next_button_{$form["id"]}_{$field["id"]}", $field["nextButton"], __("Next", "gravityforms"), "button gform_next_button btn", __("Next Page", "gravityforms"), $field["pageNumber"]);
                $previous_button = $field["pageNumber"] == 2 ? "" : self::get_form_button($form["id"], "gform_previous_button_{$form["id"]}_{$field["id"]}", $field["previousButton"], __("Previous", "gravityforms"), "button gform_previous_button btn", __("Previous Page", "gravityforms"), $field["pageNumber"]-2);
