...
function genki_announcement_showmsg() {
	...
	if (get_option('genki_announcement_multimsg') == '0') {
		if (!isset($msgShownBefore)) {
			$msgShownBefore = 'yes';
			echo '<section class="box-container" id="announce"><p>' . stripslashes(get_option('genki_announcement_msg')) . '</p></section>';
		}
	}
	else {
		global $msgShownBefore;
		...
		<form name="formamt" method="post" action="<?php echo $_SERVER["REQUEST_URI"]; ?>">
			<fieldset class="options">
				<legend><?php _e('Announcement status','genki_announce'); ?>
					<p>
					<input name="location" type="radio" id="location" value="0" <?php if ($location == '0') echo 'checked' ; ?>> <?php _e('Don\'t show announcement','genki_announce'); ?></input><br />
