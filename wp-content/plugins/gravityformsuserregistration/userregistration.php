...
class GFUser {
    ...
    public static function add_multisite_section($config, $form, $is_validation_error) {
        ...
        <div id="multsite_options" class="multsite_options">
            ...
            <h3><?php _e("Multisite Options", "userregistrationuserregistration"); ?></h3>
            ...
            <div class="margin_vertical_10">
                ...
                <input type="checkbox" id="gf_user_registration_multisite_create_site" name="gf_user_registration_multisite_create_site" value="1" <?php echo rgar($multisite_options, 'create_site') == 1 ? "checked='checked'" : "" ?> onclick="if(this.checked) { jQuery('#multisite_option_items').fadeIn('fast'); } else { jQuery('#multisite_option_items').fadeOut('fast'); }" />
            ...
            <div id="multisite_option_items" style="<?php echo rgar($multisite_options, 'create_site') == 1 ? 'display:block;' : 'display:none;'; ?>">
    ...
    public static function gf_create_user($entry, $form, $fulfilled = false) {
        ...
        // if there is no registration feed or the registration condition is not met, abandon ship
        if(!$user_config || !self::registration_condition_met($form, $user_config))
            return;
        
        // if PayPal Add-on is active, integrate
        if(class_exists('GFPayPal')) {
            $paypal_config = GFPayPal::get_config($form['id']);
        ...
        $user_name = apply_filters('gform_username_' . $form['id'], apply_filters('gform_username', self::get_prepared_value($username_field, $user_config['meta']['username'], $entry, true)));
        $user_email = self::get_prepared_value($useremail_field, $user_config['meta']['email'], $entry);
        $password = rgar($entry, rgar($user_config['meta'], 'password'));
        ...
        if(!$user_id && empty($password)) {
            ...
            self::add_usermeta($user_id, $user_config['meta'], $entry, array());
        ...
        // set post author
        if(!empty($entry['post_id']) && $user_config['meta']['set_post_author'])
    ...
    private static function add_usermeta($user_id, $meta, $entry, $name_fields) {
        ...
        // standard meta
        $firstname_field = (isset($meta['firstname'])) ? RGFormsModel::get_field($form, $meta['firstname']) : false;
        $firstname = (!empty($firstname_field)) ? self::get_prepared_value($firstname_field, $meta['firstname'], $entry) : false;
        if(!empty($firstname))
            update_user_meta($user_id, 'first_name', $firstname);

        $lastname_field = (isset($meta['lastname'])) ? RGFormsModel::get_field($form, $meta['lastname']) : false;
        $lastname = (!empty($lastname_field)) ? self::get_prepared_value($lastname_field, $meta['lastname'], $entry) : false;
        if(!empty($lastname))
        ...
        if(is_array($meta['user_meta'])) {
            ...
            foreach($meta['user_meta'] as $user_meta) {
                ...
                $field = RGFormsModel::get_field($form, $user_meta['meta_value']);
                $value = self::get_prepared_value($field, $user_meta['meta_value'], $entry);
    ...
    public static function user_registration_validation($validation_result){
        ...
        $user_name = apply_filters('gform_username_' . $form['id'], apply_filters('gform_username', self::get_prepared_value($username_field, $config['meta']['username'], $entry, true)));
        ...
        switch(RGFormsModel::get_input_type($field)){
            case "name":
                if(strpos($input_id, '.') === false){
                    $prefix = trim($entry[$input_id. '.2']);
                    $first = trim($entry[$input_id . '.3']);
                    $last = trim($entry[$input_id . '.6']);
                    $suffix = trim($entry[$input_id . '.8']);
            ...
            case "address" :
                if(strpos($input_id, '.') === false){
                    $street_value = trim($entry[$input_id . '.1']);
                    $street2_value = trim($entry[$input_id . '.2']);
                    $city_value = trim($entry[$input_id . '.3']);
                    $state_value = trim($entry[$input_id . '.4']);
                    $zip_value = trim($entry[$input_id . '.5']);
                    $country_value = trim($entry[$input_id . '.6']);
                ...
                <li id="gf_paypal_update_user_option" <?php echo $config["meta"]["type"] == "subscription" ? "" : "style='display:none;'" ?>>
                    ...
                    <select id="gf_paypal_update_user_action" name="gf_paypal_update_user_action" onchange="var checked = jQuery(this).val() ? 'checked' : false; jQuery('#gf_paypal_update_user').attr('checked', checked);">
                        ...
                        <?php foreach($roles as $role) {
                            $role_name = ucfirst($role);
                            ?>
                            <option value="<?php echo $role ?>" <?php echo $config["meta"]["update_user_action"] == $role ? "selected='selected'" : ""?>><?php echo sprintf(__("Set User as %s", "gravityformsuserregistration"), $role_name); ?></option>
                        <?php } ?>
                    </select>
                </li>
    ...
    public static function custom_registration_page() {
        ...
        wp_redirect(get_permalink(rgar($ur_settings, 'custom_reg_page')));
